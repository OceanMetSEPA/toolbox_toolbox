function [Msg,axt,lbt] = timeaxis_ucsd(varargin)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% $Workfile:   timeaxis_ucsd.m  $
% $Revision:   1.0  $
% $Author:   ted.schlicke  $
...
% Stunden
  ...
  [t,tl,l,lp,lt] = hour_tick(lim,it,il);

%------------------------------------------
elseif il < 360
% Tage des Monats

  [t,tl,l,lp,lt] = day_tick(lim,it,il,it0,il0);
...
%Command to Hide the Toolbar
% set( fig , 'currentaxes' , curr_axe , ...
%            'toolbar'     , 'none'          );

%Command to Display the Toolbar
set( fig , 'currentaxes' , curr_axe , ...
           'toolbar'     , 'figure'          );
...
function   [t,tl,l,lp,lt] = hour_tick(lim,it,il);
...
function   [t,tl,l,lp,lt] = day_tick(lim,it,il,it0,il0);
...
for ii = 1 : nl
  ...
  form = cat( 2 , '%2.2d.' , mon(dl(ii,1),:) );
 
  l{ii} = sprintf(form,dl(ii,2));
...
%   $Revision:   1.0  $  $Date:   Apr 08 2014 14:02:52  $
